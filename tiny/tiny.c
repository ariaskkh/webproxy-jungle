...
void serve_dynamic(int fd, char *filename, char *cgiargsm, char *method);
...
// 입력 ./tiny 3333 / argc = 2 / argv[0] = tiny, argv[1] = 3333
int main(int argc, char **argv) {
    ...
    while (1) {
        clientlen = sizeof(clientaddr); //Accept에 넣기 위해 주소 길이 계산
...
void doit(int fd) {
    ...
    if (is_static) { //serve static content
        ...
        // S_IRUSR -> 읽기 권한이 있는지 
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) { //일반 파일 아니거나 읽기 권한 없는 경우
...
// strcmp : 문자열 왼쪽 크면 1, 같으면 0, 오른쪽 크면 -1
void read_requesthdrs(rio_t *rp) {
    char buf[MAXLINE];
...
void serve_static(int fd, char *filename, int filesize, char *method) {
    ...
    sprintf(buf, "%sServer: Tiny Web Server\r\n", buf); // 이하 response headers
    sprintf(buf, "%sConnection: close\r\n", buf); 
    sprintf(buf, "%sContent-length: %d\r\n", buf, filesize);
    sprintf(buf, "%sContent-type: %s\r\n\r\n", buf, filetype); //빈줄 한 개 나오면 헤더 종료
    Rio_writen(fd, buf, strlen(buf));
    ...
    srcfd = Open(filename, O_RDONLY, 0); //fd 값을 얻음

    // mmap는 요청한 파일을 가상메모리 영역(디스크)으로 매핑함
    ...
    // srcp = Mmap(0, filesize, PROT_READ, MAP_PRIVATE, srcfd, 0); // start address 반환
    
    srcp = (char *)Malloc(filesize); // mmap 대신 malloc 쓰기 위해 2줄 추가
    Rio_readn(srcfd, srcp, filesize);

    Close(srcfd); // 파일을 메모리에 일단 매핑하면 srcfd(descriptor)는 더이상 필요 없다.
    // 파일을 client에게 전송 -> 주소 srcp에서 시작하는 filesize 바이트를 클라이언트의 연결 식별자로 복사
    Rio_writen(fd, srcp, filesize); 
    // int munmap(void *addr, size_t len);
    // munmap 함수는 addr이 가리키는 영역에 len 크기만큼 할당하여 매핑한 메모리를 해제함, mmap 후 free해주는 개념
    // 매핑된 가상메모리 주소 반환
...
void serve_dynamic(int fd, char *filename, char *cgiargs, char *method) {
    ...
    sprintf(buf, "HTTP/1.0 200 OK\r\n"); // response line -> success to the client
    Rio_writen(fd, buf, strlen(buf));
    ...
    if (Fork() == 0) { // 자식 프로세스  //child
        ...
        setenv("QUERY_STRING", cgiargs, 1); // QUERY_SPRING을 "15000&213"으로 설정
